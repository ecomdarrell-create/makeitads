...
import { ArrowRight, Eye, Check, X, ChevronDown, Loader2 } from "lucide-react";
...
const howItWorksSteps = [
  { number: "01", title: "Tell us about your business", description: "Describe your company, products, audience and goals in plain English. No prompts or technical setup required.", image: "/images/step1-tell-business.webp" },
  { number: "02", title: "Analyze the market and competitors", description: "MakeItAds studies your competitors, positioning, trends and opportunities to build a complete market overview for your next growth move.", image: "/images/howitworks-step2.webp" },
  { number: "03", title: "Receive a complete growth strategy", description: "Get a structured marketing plan including positioning, customer personas, acquisition channels and actionable recommendations.", image: "/images/step3-growth-strategy.webp" },
  { number: "04", title: "Turn insights into campaigns", description: "Transform your strategy into marketing campaigns, content ideas and creative briefs ready to execute.", image: "/images/step4-launch-campaigns.webp" },
];
      ...
      <div className="max-w-7xl mx-auto">
        {howItWorksSteps.map((step, index) => (
          <div key={step.number} className="relative">
            <motion.div initial={{ opacity: 0, y: 60 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, margin: "-100px" }} transition={{ duration: 0.8, delay: index * 0.2 }} className="relative min-h-[70vh] lg:min-h-screen flex items-center py-16 lg:py-20 px-4 sm:px-6">
              <div className="absolute inset-0 z-0">
                <Image src={step.image} alt={step.title} fill className="object-cover" sizes="(max-width: 768px) 100vw, 50vw" priority={index === 0} />
                ...
                <div className={`grid md:grid-cols-2 gap-8 lg:gap-12 items-center ${index % 2 === 1 ? "md:flex-row-reverse" : ""}`}>
                  <div className={`text-center md:text-left ${index % 2 === 1 ? "md:order-2" : "md:order-1"}`}>
                    <div className="relative">
                      <span className="text-[80px] sm:text-[120px] md:text-[160px] font-bold text-[#6366f1]/10 absolute -top-4 -left-2 select-none leading-none">{step.number}</span>
                      <motion.h3 initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.5, delay: 0.3 }} className="text-2xl sm:text-3xl md:text-4xl lg:text-5xl font-bold text-[#0F172A] mb-4 sm:mb-6 leading-[1.1] tracking-[-0.04em] relative z-10 pt-8 sm:pt-12">{step.title}</motion.h3>
                      <motion.p initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.5, delay: 0.4 }} className="text-sm sm:text-base md:text-lg text-[#475569] leading-relaxed relative z-10">{step.description}</motion.p>
                    </div>
                  </div>
      ...
      <div className="relative py-20 sm:py-24 px-6 bg-gradient-to-b from-[#F8FAFC] via-[#FFFFFF] to-[#F8FAFC]">
        <div className="relative z-10 max-w-4xl mx-auto text-center">
          <motion.div initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.6 }} className="flex justify-center">
            <Link href="#pricing" className="group inline-flex items-center gap-2 rounded-full bg-[#6366f1] px-8 sm:px-10 py-4 text-sm font-bold text-white shadow-[0_0_40px_-10px_rgba(99,102,241,0.5)] hover:bg-[#5558e6] transition-all hover:scale-105">
              Voir les offres <ArrowRight className="h-4 w-4 group-hover:translate-x-1 transition-transform" />
            </Link>
          </motion.div>
        </div>
      </div>
      ...
      {painPoints.map((point) => (
        <div key={point.id} className={`flex-shrink-0 w-[290px] sm:w-[320px] md:w-[380px] snap-start group relative rounded-2xl sm:rounded-3xl overflow-hidden border ${point.borderColor} bg-white shadow-lg hover:shadow-2xl hover:shadow-[#6366f1]/10 transition-all duration-500`}>
          <div className="aspect-[16/10] overflow-hidden relative">
            <Image src={point.image} alt={point.title} fill className="object-cover transition duration-700 group-hover:scale-110" sizes="(max-width: 640px) 290px, (max-width: 768px) 320px, 380px" />
            {point.glassmorphism && (<>
...
interface FeatureRow { name: string; makeitads: CellValue; chatgpt: CellValue; claude: CellValue; google: CellValue; meta: CellValue; }
...
const tools = [ { key: "makeitads", name: "MakeItAds", isPrimary: true }, { key: "chatgpt", name: "Generic AI", isPrimary: false }, { key: "claude", name: "Generic AI", isPrimary: false }, { key: "google", name: "Search Trends", isPrimary: false }, { key: "meta", name: "Platform Signals", isPrimary: false } ];

function CellValueRenderer({ value, isPrimary }: { value: CellValue; isPrimary: boolean }) {
  if (value === "yes") return <div className={`flex items-center justify-center ${isPrimary ? "h-8 w-8 rounded-full bg-emerald-500/20" : ""}`}><Check className={`h-4 w-4 sm:h-5 sm:w-5 ${isPrimary ? "text-emerald-400" : "text-emerald-400"}`} strokeWidth={2.5} /></div>;
  if (value === "no") return <div className="flex items-center justify-center"><X className="h-4 w-4 sm:h-5 sm:w-5 text-[#CBD5E1]" strokeWidth={2.5} /></div>;
  if (value === "partial") return <span className="inline-flex items-center rounded-md border border-amber-500/30 bg-amber-500/10 px-2 py-0.5 text-[10px] sm:text-xs font-semibold text-amber-600">Partial</span>;
  if (value === "basic" || value === "manual") return <span className="inline-flex items-center rounded-md border border-[#CBD5E1] bg-[#F1F5F9] px-2 py-0.5 text-[10px] sm:text-xs font-semibold text-[#64748B]">{value === "basic" ? "Basic" : "Manual"}</span>;
  return <span className="text-[10px] sm:text-xs text-[#94A3B8] capitalize">{value}</span>;
}

function ComparisonSection() {
  return (
    <section className="relative z-10 py-20 sm:py-24 md:py-32 px-4 sm:px-6 overflow-hidden bg-[#F8FAFC]">
      <div className="absolute inset-0 pointer-events-none"><div className="absolute top-0 left-1/2 -translate-x-1/2 w-[800px] h-[400px] bg-[#6366f1]/5 rounded-full blur-[120px]" /></div>
      <div className="relative z-10 max-w-7xl mx-auto">
        <motion.div initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.6 }} className="text-center mb-12 sm:mb-16 md:mb-20 max-w-4xl mx-auto px-4">
          <h2 className="text-2xl sm:text-3xl md:text-4xl lg:text-5xl font-bold tracking-[-0.04em] leading-[1.1] mb-4 sm:mb-6 text-[#0F172A] text-left sm:text-center">A <span className="text-[#8b5cf6]">marketing strategy engine</span> for teams that need to move faster than the market.</h2>
          <p className="text-sm sm:text-base md:text-lg text-[#475569] leading-relaxed max-w-2xl text-left sm:text-center">Instead of stitching together scattered tools, teams get one operating layer for market analysis, competitor tracking and campaign direction.</p>
        </motion.div>
        <motion.div initial={{ opacity: 0, y: 40 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, margin: "-50px" }} transition={{ duration: 0.7 }} className="hidden lg:block rounded-2xl border border-[#E2E8F0] bg-white shadow-[0_10px_40px_rgba(15,23,42,0.08)] overflow-hidden">
            ...
            <table className="w-full">
              <thead>
                <tr className="border-b border-[#E2E8F0]">
                  ...
                  {tools.map((tool) => (<th key={tool.key} className={`px-6 py-5 text-center text-xs font-semibold uppercase tracking-wider ${tool.isPrimary ? "text-[#6366f1] bg-[#EEF2FF] border-l border-r border-[#6366f1]/20" : "text-[#64748B]"}`}>{tool.name}</th>))}
                </tr>
              </thead>
              <tbody>
        ...
        <motion.div initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.6, delay: 0.2 }} className="mt-16 sm:mt-20 md:mt-24 text-center max-w-3xl mx-auto px-4">
          <h3 className="text-xl sm:text-2xl md:text-3xl font-bold text-[#0F172A] mb-3 sm:mb-4 leading-[1.2] text-left sm:text-center">One workspace for building the next <span className="text-[#8b5cf6]">marketing strategy</span> with clarity.</h3>
          <p className="text-sm sm:text-base text-[#475569] mb-6 sm:mb-8 leading-relaxed text-left sm:text-center">Replace fragmented workflows with one layer for market intelligence, competitor understanding and execution-ready campaign direction.</p>
          <div className="flex flex-col sm:flex-row items-center justify-center gap-3 sm:gap-4">
            <Link href="#pricing" className="group inline-flex items-center justify-center gap-2 rounded-full bg-[#6366f1] px-6 sm:px-8 py-3 text-sm font-bold text-white shadow-[0_0_40px_-10px_rgba(99,102,241,0.5)] hover:bg-[#5558e6] transition-all hover:scale-105 w-full sm:w-auto">Voir les offres <ArrowRight className="h-4 w-4 group-hover:translate-x-1 transition-transform" /></Link>
          </div>
        </motion.div>
      </div>
    </section>
...
const partnerLogos = [ { name: "Shopify", icon: SiShopify }, { name: "Stripe", icon: SiStripe }, { name: "Meta", icon: SiMeta }, { name: "Google", icon: SiGoogle }, { name: "HubSpot", icon: SiHubspot }, { name: "TikTok", icon: SiTiktok } ];
const faqData = [
  ...
  { question: "Is there a free plan?", answer: "Yes! We offer a free initial strategy analysis to prove our value." },
  { question: "How long does it take to generate a strategy?", answer: "Our experts deliver your personalized PDF strategy within 24 hours." },
  { question: "Which ad platforms do you support?", answer: "We specialize in Meta (Facebook/Instagram), TikTok, and Google Ads, calibrated for the African market." },
];
  ...
  <div className="relative overflow-hidden">
    <div className="flex animate-[scroll_15s_linear_infinite] hover:[animation-play-state:paused]">
      {[...partnerLogos, ...partnerLogos, ...partnerLogos].map((logo, index) => {
        ...
        return (<div key={index} className="flex-shrink-0 mx-8 flex items-center justify-center"><Icon className="w-10 h-10 text-[#94A3B8] hover:text-[#6366f1] transition-colors duration-300" /></div>);
      })}
    </div>
  ...
  return (
    <main className="min-h-screen bg-[#FFFFFF] text-[#0F172A] overflow-hidden selection:bg-[#6366f1]/20 selection:text-[#0F172A]">
        ...
        <div className="max-w-7xl mx-auto">
          <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.6 }} className="mb-10 md:mb-16 max-w-5xl">
            <h2 className="text-3xl sm:text-4xl md:text-5xl lg:text-6xl font-bold tracking-[-0.04em] leading-[1.1] text-left text-[#0F172A]">
              ...
              <span className="block mt-2 text-[#8b5cf6]">They're missing the right <span className="text-[#8b5cf6]">market angle</span>, the right <span className="text-[#8b5cf6]">competitor signal</span>, and the right <span className="text-[#8b5cf6]">growth strategy</span>.</span>
            </h2>
            <p className="mt-4 max-w-2xl text-left text-sm sm:text-base md:text-lg text-[#475569] leading-relaxed">When the signal is fragmented, your team reacts late, spends inefficiently, and loses the speed advantage that premium growth brands need.</p>
          </motion.div>
          <PainPointsCarousel />
        </div>
      </section>
        ...
        <div className="max-w-7xl mx-auto text-center">
          <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.6 }}>
            <h2 className="text-3xl md:text-5xl font-bold tracking-[-0.04em] leading-[1.1] mb-4 text-[#0F172A]">
              Simple, transparent <span className="bg-gradient-to-r from-[#6366f1] via-[#8b5cf6] to-[#38bdf8] bg-clip-text text-transparent">pricing</span>
            </h2>
            <p className="text-base md:text-lg text-[#475569] mb-8">Choose the plan that fits your business needs.</p>
            <p className="text-sm text-[#64748B]">Redirection vers les offres Chariow configurées dans la page principale.</p>
          </motion.div>
        </div>
      </section>
      ...
      <section id="faq" className="relative z-10 py-16 md:py-24 px-6 bg-[#F8FAFC]">
        <div className="max-w-3xl mx-auto">
          <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.6 }} className="text-center mb-12">
            <h2 className="text-3xl md:text-5xl font-bold tracking-[-0.04em] leading-[1.1] mb-4 text-[#0F172A]">Frequently asked <span className="bg-gradient-to-r from-[#6366f1] via-[#8b5cf6] to-[#38bdf8] bg-clip-text text-transparent">questions</span></h2>
                ...
                <AnimatePresence>
                  {openFaq === index && (
                    <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: "auto", opacity: 1 }} exit={{ height: 0, opacity: 0 }} transition={{ duration: 0.3 }} className="overflow-hidden">
                      <div className="px-6 pb-6"><p className="text-sm text-[#475569] leading-relaxed">{faq.answer}</p></div>
                    </motion.div>
                  )}
                </AnimatePresence>
              </motion.div>
      ...
      <section className="relative z-10 py-24 md:py-32 px-6 overflow-hidden bg-gradient-to-b from-[#FFFFFF] via-[#F8FAFC] to-[#FFFFFF]">
        <div className="absolute inset-0 z-0"><div className="absolute top-0 left-1/2 -translate-x-1/2 w-[1000px] h-[600px] bg-[#6366f1]/5 rounded-full blur-[120px]" /></div>
        <div className="relative z-10 max-w-4xl mx-auto text-center">
          <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.6 }}>
            <h2 className="text-3xl md:text-6xl font-bold tracking-[-0.04em] leading-[1.1] mb-6 text-[#0F172A]">Stop making decisions <span className="bg-gradient-to-r from-[#6366f1] via-[#8b5cf6] to-[#38bdf8] bg-clip-text text-transparent">in the dark.</span></h2>
            ...
            <button onClick={scrollToPricing} className="group inline-flex items-center gap-2 rounded-full bg-[#6366f1] px-8 md:px-12 py-4 md:py-5 text-base md:text-lg font-bold text-white shadow-[0_0_60px_-10px_rgba(99,102,241,0.6)] hover:bg-[#5558e6] transition-all hover:scale-105">
              Voir les offres <ArrowRight className="h-5 w-5 group-hover:translate-x-1 transition-transform" />
            </button>
          </motion.div>
        </div>
      </section>
