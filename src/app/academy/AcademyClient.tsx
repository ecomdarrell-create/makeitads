  ...
  const filteredArticles = useMemo(() => {
    ...
    switch (selectedFilter) {
      ...
      case "trending":
        ...
        break;
      case "editors-picks":
        result = result.filter((a) => a.editorsPick || a.featured);
        break;
      case "newest":
        ...
        <div className="relative max-w-7xl mx-auto px-6 py-20 md:py-28">
          ...
          >
            ...
            <p className="text-lg md:text-xl text-slate-600 dark:text-slate-400 leading-relaxed">
              Expert guides, proven strategies, and AI insights from the world's top marketers. Learn what's working now in 2026.
            </p>
          </motion.div>
        </div>
      </section>
        ...
        <section className="max-w-7xl mx-auto px-6 py-12">
          ...
          >
            <Link href={`/academy/${featuredArticle.slug}`} className="group block">
              <div className="grid md:grid-cols-2 gap-8 rounded-3xl border border-black/5 dark:border-white/10 bg-white dark:bg-white/[0.02] overflow-hidden hover:shadow-2xl hover:shadow-[#6366f1]/5 transition-all duration-500">
                <div className="relative aspect-[16/10] md:aspect-auto overflow-hidden">
                  ...
                  <div className="absolute top-4 left-4 flex items-center gap-2">
                    <span className="rounded-full bg-white/90 dark:bg-black/80 backdrop-blur-md px-3 py-1 text-xs font-bold text-slate-900 dark:text-white shadow-sm">
                      Featured
                    </span>
                    <span className="rounded-full bg-[#6366f1] px-3 py-1 text-xs font-bold text-white shadow-sm">
                ...
                <div className="p-8 md:p-10 flex flex-col justify-center">
                  ...
                  <h2 className="text-2xl md:text-3xl font-bold text-slate-900 dark:text-white mb-4 group-hover:text-[#6366f1] transition-colors">
        ...
        <div className="flex flex-col lg:flex-row gap-4 mb-8">
          <div className="relative flex-1">
            ...
            <input
              ...
              className="w-full rounded-2xl border border-black/10 dark:border-white/10 bg-white dark:bg-white/[0.02] pl-11 pr-4 py-3.5 text-sm text-slate-900 dark:text-white placeholder:text-slate-400 outline-none focus:border-[#6366f1] focus:ring-4 focus:ring-[#6366f1]/10 transition-all"
            />
          </div>
          ...
          <button
            ...
            className={`flex-shrink-0 rounded-full px-4 py-2 text-xs font-semibold transition-all ${
              selectedCategory === "All"
                ? "bg-slate-900 dark:bg-white text-white dark:text-slate-900"
                : "border border-black/10 dark:border-white/10 text-slate-600 dark:text-slate-400 hover:border-slate-900 dark:hover:border-white"
            }`}
          >
            All topics
          </button>
          ...
          {FILTERS.map((filter) => {
            ...
            return (
              <button
                ...
                className={`flex items-center gap-2 flex-shrink-0 rounded-full px-4 py-2 text-xs font-semibold transition-all ${
                  selectedFilter === filter.id
                    ? "bg-[#6366f1] text-white"
                    : "border border-black/10 dark:border-white/10 text-slate-600 dark:text-slate-400 hover:border-[#6366f1] hover:text-[#6366f1]"
                }`}
              >
      ...
      <section className="max-w-7xl mx-auto px-6 pb-20">
        <div className="flex items-center justify-between mb-8">
          <p className="text-sm text-slate-500">
            <span className="font-semibold text-slate-900 dark:text-white">{filteredArticles.length}</span>{" "}
        ...
        {filteredArticles.length === 0 ? (
          ...
          >
            <div className="inline-flex h-16 w-16 items-center justify-center rounded-2xl bg-slate-100 dark:bg-white/5 mb-4">
        ...
        <div className="max-w-3xl mx-auto px-6 py-20 text-center">
          ...
          >
            ...
            <p className="text-lg text-slate-600 dark:text-slate-400 mb-8">
              Receive new marketing strategies and AI insights every week. No spam, unsubscribe anytime.
            </p>
            <form className="flex flex-col sm:flex-row gap-3 max-w-md mx-auto">
              <input
                type="email"
                placeholder="you@company.com"
                className="flex-1 rounded-2xl border border-black/10 dark:border-white/10 bg-white dark:bg-white/[0.02] px-5 py-3.5 text-sm text-slate-900 dark:text-white placeholder:text-slate-400 outline-none focus:border-[#6366f1] focus:ring-4 focus:ring-[#6366f1]/10 transition-all"
              />
              <button
                type="button"
                className="rounded-2xl bg-slate-900 dark:bg-white px-6 py-3.5 text-sm font-semibold text-white dark:text-slate-900 hover:opacity-90 transition-all"
              >
                Subscribe
              </button>
            </form>
            <p className="text-xs text-slate-500 mt-4">
              Join 12,000+ marketers. Free forever.
            </p>
          </motion.div>
        </div>
      </section>
